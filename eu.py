...
def construir_features(self, numeros):
    ...
    # 4. Dúzia mais quente nas últimas 10
    ...
    # 5. Repetição do número
    repetiu_numero = int(atual == val1) if len(anteriores) >= 1 else 0

    # 6. Vizinho do anterior (±2)
    vizinho = int(abs(atual - val1) <= 2) if len(anteriores) >= 1 else 0

    # 7. Quadrante da roleta física (dividido em 4 partes arbitrárias)
    if atual in range(1, 10):
        quadrante_roleta = 0
    elif atual in range(10, 19):
        quadrante_roleta = 1
    elif atual in range(19, 28):
        quadrante_roleta = 2
    elif atual in range(28, 37):
        quadrante_roleta = 3
    else:
        quadrante_roleta = -1

    # 8. Número frequente no histórico total (top 5)
    top5_freq = [n for n, _ in Counter(numeros).most_common(5)]
    numero_frequente = int(atual in top5_freq)

    # 9. Densidade por faixa (1–6, 7–12, ..., 31–36)
...
# Previsões com base no histórico atual (antes de adicionar novo)
...
# Votação ponderada
votacao = Counter()
if prev_ia is not None: votacao[prev_ia] += pesos["ia"]
if prev_quente is not None: votacao[prev_quente] += pesos["quente"]
if prev_tendencia is not None: votacao[prev_tendencia] += pesos["tendencia"]
if prev_alternancia is not None: votacao[prev_alternancia] += pesos["alternancia"]
if prev_ausente is not None: votacao[prev_ausente] += pesos["ausente"]

mais_votado, _ = votacao.most_common(1)[0]
st.session_state.duzia_prevista = mais_votado

# Auto atualização e captura
