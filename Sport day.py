...
# =============================
# Utilitários de Data e Formatação - CORREÇÃO APLICADA
# =============================
def formatar_data_iso(data_iso: str) -> tuple[str, str]:
    """Formata data ISO de forma robusta - CORREÇÃO APLICADA"""
    try:
        # CORREÇÃO: Converter para timezone UTC e depois para horário de Brasília (UTC-3)
        if data_iso.endswith('Z'):
            data_iso = data_iso[:-1] + '+00:00'
        
        # Criar objeto datetime com timezone UTC
        data_utc = datetime.fromisoformat(data_iso)
        
        # CORREÇÃO: Converter para horário de Brasília (UTC-3)
        # A API retorna em UTC, então subtraímos 3 horas para BRT
...
# =============================
# ALERTAS TELEGRAM PARA NOVAS PREVISÕES - CORREÇÃO APLICADA
# =============================

def enviar_alerta_telegram_ambas_marcam(fixture: dict, tendencia: str, probabilidade: float, confianca: float) -> bool:
    """Envia alerta individual para Ambas Marcam - CORREÇÃO APLICADA"""
    home = fixture["homeTeam"]["name"]
    away = fixture["awayTeam"]["name"]
    
    # CORREÇÃO APLICADA: Garantir que a data seja formatada corretamente
...
def enviar_alerta_telegram_cartoes(fixture: dict, tendencia: str, estimativa: float, confianca: float) -> bool:
    """Envia alerta individual para Cartões - CORREÇÃO APLICADA"""
    home = fixture["homeTeam"]["name"]
    away = fixture["awayTeam"]["name"]
    
    # CORREÇÃO APLICADA: Garantir formatação correta da data
    data_formatada, hora_formatada = formatar_data_iso(fixture["utcDate"])
...
def enviar_alerta_telegram_escanteios(fixture: dict, tendencia: str, estimativa: float, confianca: float) -> bool:
    """Envia alerta individual para Escanteios - CORREÇÃO APLICADA"""
    home = fixture["homeTeam"]["name"]
    away = fixture["awayTeam"]["name"]
...
def enviar_alerta_resultados_compostos_poster(jogos_com_resultado: list):
    ...
    try:
        ...
        jogos_por_data = {}
        for jogo in jogos_com_resultado:
            # CORREÇÃO APLICADA: Usar a função formatar_data_iso corrigida
...
def verificar_resultados_ambas_marcam(alerta_resultados: bool):
    """Verifica resultados para previsão Ambas Marcam - CORREÇÃO APLICADA"""
    alertas = carregar_alertas_ambas_marcam()
    if not alertas:
...
def verificar_resultados_cartoes(alerta_resultados: bool):
    """Verifica resultados para previsão de Cartões - CORREÇÃO APLICADA"""
    alertas = carregar_alertas_cartoes()
    if not alertas:
...
def verificar_resultados_escanteios(alerta_resultados: bool):
    """Verifica resultados para previsão de Escanteios - CORREÇÃO APLICADA"""
    alertas = carregar_alertas_escanteios()
    if not alertas:
...
# =============================
# SISTEMA DE ALERTAS DE RESULTADOS ORIGINAL - CORREÇÃO APLICADA
# =============================

def verificar_resultados_finais(alerta_resultados: bool):
    """Verifica resultados finais dos jogos e envia alertas - CORREÇÃO APLICADA"""
    alertas = carregar_alertas()
    if not alertas:
...
def gerar_poster_multiplos_jogos(jogos: list, titulo: str = "ELITE MASTER - ALERTAS DO DIA") -> io.BytesIO:
    ...
    for idx, jogo in enumerate(jogos):
        ...
        # CORREÇÃO APLICADA: Usar dados formatados já calculados
        if 'hora_formatada' in jogo and 'data_formatada' in jogo:
            hora_text = f"HORÁRIO: {jogo['hora_formatada']} BRT | DATA: {jogo['data_formatada']}"
        else:
            try:
                # Usar a função formatar_data_iso corrigida
                data_formatada, hora_formatada = formatar_data_iso(jogo.get("fixture", {}).get("utcDate", ""))
                hora_text = f"HORÁRIO: {hora_formatada} BRT | DATA: {data_formatada}"
            except:
...
def enviar_alerta_composto_poster(jogos_conf: list, threshold: int):
    """Envia alerta composto com poster para múltiplos jogos - CORREÇÃO APLICADA"""
    if not jogos_conf:
...
def enviar_alerta_composto_texto(jogos_conf: list, threshold: int) -> bool:
    """Fallback para alerta composto em texto - CORREÇÃO APLICADA"""
    try:
        msg = f"🔥 Jogos ≥{threshold}% (Estilo Original):\n\n"
        
        for jogo in jogos_conf:
            # CORREÇÃO APLICADA: Usar dados formatados se disponíveis
            if 'hora_formatada' in jogo and 'data_formatada' in jogo:
                ...
                # Usar a função formatar_data_iso corrigida
                try:
                    fixture_data = jogo.get('fixture', {}).get('utcDate', '')
                    data_formatada, hora_formatada = formatar_data_iso(fixture_data)
                    hora_text = hora_formatada
                    data_text = data_formatada
                except:
                    hora_text = "Hora inválida"
                    data_text = "Data inválida"
...
def enviar_alerta_telegram(fixture: dict, tendencia: str, estimativa: float, confianca: float):
    """Envia alerta individual com poster estilo West Ham - CORREÇÃO APLICADA"""
    try:
...
def enviar_alerta_telegram_fallback(fixture: dict, tendencia: str, estimativa: float, confianca: float) -> bool:
    """Fallback para alerta em texto caso o poster falhe - CORREÇÃO APLICADA"""
    home = fixture["homeTeam"]["name"]
    away = fixture["awayTeam"]["name"]
    # CORREÇÃO APLICADA: Usar a função formatar_data_iso corrigida
    data_formatada, hora_formatada = formatar_data_iso(fixture["utcDate"])
...
def gerar_poster_resultados(jogos: list, titulo: str = "ELITE MASTER - RESULTADOS OFICIAIS") -> io.BytesIO:
    ...
    for idx, jogo in enumerate(jogos):
        ...
        # Data do jogo - CORREÇÃO APLICADA
...
def enviar_alerta_resultados_poster(jogos_com_resultado: list):
    """Envia alerta de resultados com poster para o Telegram - CORREÇÃO APLICADA"""
    if not jogos_com_resultado:
...
# =============================
# FUNÇÕES PRINCIPAIS - CORREÇÃO APLICADA
# =============================

def enviar_top_jogos(jogos: list, top_n: int, alerta_top_jogos: bool):
    """Envia os top jogos para o Telegram - CORREÇÃO APLICADA"""
    ...
    for j in top_jogos_sorted:
        ...
        if 'hora_formatada' in j and 'data_formatada' in j:
            ...
            # Usar a função formatar_data_iso corrigida
            try:
                fixture_data = j.get('fixture', {}).get('utcDate', '')
                data_formatada, hora_formatada = formatar_data_iso(fixture_data)
                hora_text = hora_formatada
                data_text = data_formatada
            except:
...
# =============================
# PROCESSAMENTO PRINCIPAL ATUALIZADO - COM SELEÇÃO MÚLTIPLA E CORREÇÃO APLICADA
# =============================
...
def processar_jogos_avancado(data_selecionada, todas_ligas, ligas_selecionadas, top_n, 
    ...
    """Processamento AVANÇADO com dados REAIS da API - CORREÇÃO APLICADA"""
    ...
    for i, liga_id in enumerate(ligas_busca):
        ...
        for match in jogos:
            ...
            # CORREÇÃO APLICADA: Formatar data/hora UMA VEZ e reutilizar
            ...
            # Dados para previsão original de gols - CORREÇÃO APLICADA
            jogo_data = {
                "id": match["id"],
                "home": home_name,
                "away": away_name,
            ...
            # NOVAS PREVISÕES COM DADOS REAIS - CORREÇÃO APLICADA
            # Ambas Marcam
            ...
            # Cartões COM DADOS REAIS - CORREÇÃO APLICADA
            if alerta_cartoes:
            ...
            # Escanteios COM DADOS REAIS - CORREÇÃO APLICADA
    ...
    # ENVIO DE ALERTAS COMPOSTOS - CORREÇÃO APLICADA
    if alerta_poster:
    ...
    # ENVIO DE TOP JOGOS - CORREÇÃO APLICADA
...
def exibir_resultados_ambas_marcam(jogos: list, threshold: int):
    ...
    for jogo in sorted(jogos, key=lambda x: x["confianca"], reverse=True):
        # CORREÇÃO APLICADA: Usar dados formatados
        hora_display = jogo.get('hora_formatada', 'Hora inválida')
        data_display = jogo.get('data_formatada', 'Data inválida')
