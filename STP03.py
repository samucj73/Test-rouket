...
if len(historico) >= 14:
    ...
    if prob > 0.75 and not st.session_state.entrada_atual:
        terminais = [n % 10 for n in janela]
        contagem = Counter(terminais)
        dominantes = [t for t, _ in contagem.most_common(2)]

        entrada_principal_completa = [n for n in range(37) if n % 10 in dominantes]

        # NOVA LÓGICA: Selecionar os 2 núcleos com maior probabilidade
        probs_numeros = []
        for n in entrada_principal_completa:
            janela_simulada = janela[:-1] + [n]
            features_simuladas = extrair_features(janela_simulada)
        ...
        if chave_alerta not in st.session_state.alertas_enviados:
            ...
            mensagem = (
                ...
                f"Núcleos escolhidos: {nucleos_filtrados}\n"
                f"Entrada final: {entrada_expandida}"
            )
