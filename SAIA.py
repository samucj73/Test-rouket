...
def prever_terminais(modelo, historico):
    if not modelo or len(historico) < 12:
        return []
    entrada = [n % 10 for n in list(historico)[-12:]]
...
if modelo:
    ...
    if terminais_previstos and terminais_previstos[0][1] >= PROBABILIDADE_MINIMA:
        ...
        # === ALERTA SE NOVA BASE ===
        if ultimo_alerta.get("referencia") != historico[-2]:
