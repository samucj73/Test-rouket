...
if "ultimo_treino" not in st.session_state:
    st.session_state.ultimo_treino = 0  # índice do último treino realizado

# Função para treinar se necessário
def tentar_treinar():
    historico = st.session_state.historico
    modelo = st.session_state.modelo_duzia

    if len(historico) >= modelo.janela:
        # Só treina se o histórico cresceu depois do último treino
...
if resultado and resultado["timestamp"] != ultimo:
    ...
    tentar_treinar()  # só treina se histórico tiver o mínimo e for novo

    # Previsão com modelo já treinado
    prev_ia = st.session_state.modelo_duzia.prever(st.session_state.historico)
    st.session_state.duzia_prevista = prev_ia

    # Verifica acerto da dúzia
...
# Se ainda não atualizou com novo número, faz previsão com o que já tem
if "duzia_prevista" not in st.session_state or st.session_state.duzia_prevista is None:
    tentar_treinar()
    prev_ia = st.session_state.modelo_duzia.prever(st.session_state.historico)
    st.session_state.duzia_prevista = prev_ia
else:
...
# Outras estratégias continuam iguais
