...
defaults = {
    ...
    "previsao_para_conferir": [],  # Previsões que serão conferidas
    "previsao_topN_para_conferir": [],  # Previsões TopN que serão conferidas
    "aguardando_resultado": False,  # Controla se está aguardando resultado
    "aguardando_resultado_topN": False,  # Controla se está aguardando resultado TopN
    "acertos": 0,
    "erros": 0,
    "acertos_topN": 0,
    "erros_topN": 0,
...
if resultado and novo_sorteio:
    ...
    # -----------------------------
    # 3. RESULTADO: Conferir previsão anterior e enviar GREEN/RED
    # -----------------------------
    if st.session_state.aguardando_resultado and st.session_state.previsao_para_conferir:
        ...
        st.session_state.aguardando_resultado = False  # 4. REINÍCIO: Libera para próxima previsão
    ...
    # -----------------------------
    # 1. PREVISÃO: Gerar nova previsão (apenas se não está aguardando resultado)
    # -----------------------------
    if not st.session_state.aguardando_resultado and st.session_state.contador_rodadas % 2 == 0:
