...
def criar_features_avancadas(self, historico):
    """Cria features avançadas para treinamento do XGBoost usando TODO o histórico disponível"""
    numeros = [h['number'] for h in historico if h.get('number') is not None]
    
    # Garantir quantidade mínima razoável
    if len(numeros) < 30:
        return [], []
    ...
    # Agora percorre TODO o histórico (a partir do 10º número)
    for i in range(10, len(numeros) - 1):
        feature_row = []
        janela = numeros[i-10:i]  # Últimos 10 números antes do atual
    ...
    logging.info(f"📊 XGBoost: Geradas {len(features)} amostras com {len(feature_row)} features (usando {len(numeros)} números no total)")
