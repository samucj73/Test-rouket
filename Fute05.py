    ...
    if st.button("🔍 Buscar jogos do dia e enviar Top3 (cada faixa uma mensagem)"):
        with st.spinner("Buscando jogos e calculando probabilidades..."):
            ...
            if not jogos_do_dia:
                ...
                def selecionar_top3_distintos(lista, prob_key, times_excluidos=set()):
                    selecionados = []
                    for j in sorted(lista, key=lambda x: (x[prob_key], x[f"conf_{prob_key}"], x["estimativa"]), reverse=True):
                        if j["home"] not in times_excluidos and j["away"] not in times_excluidos:
                            selecionados.append(j)
                            times_excluidos.update([j["home"], j["away"]])
