...
# === INTERFACE ===
st.title("🎯 IA Roleta - Dúzia + Coluna (RF + Features Dinâmicas)")
...
def treinar_modelos_rf():
    ...
    if len(yd)>1 and len(set(yd))>1:
        modelo_d = CatBoostClassifier(iterations=200, depth=6, learning_rate=0.1, loss_function='MultiClass', verbose=False)
        modelo_d.fit(Xd, yd)
    ...
    if len(yc)>1 and len(set(yc))>1:
        modelo_c = CatBoostClassifier(iterations=200, depth=6, learning_rate=0.1, loss_function='MultiClass', verbose=False)
        modelo_c.fit(Xc, yc)
        st.session_state.modelo_rf_coluna = modelo_c

# === PREVISÃO COM AJUSTE DINÂMICO ===
def prever_duzia_coluna_rf():
    ...
    if st.session_state.modelo_rf_duzia:
        try:
            ...
            # frequência no histórico da janela
            freq = [janela.count(classes_d[i]) for i in top_idx]
            duzia = classes_d[top_idx[np.argmin(freq)]]  # escolhe a que apareceu menos recentemente
...
if st.session_state.ultimo_resultado_numero!=numero_atual:
    ...
    if (st.session_state.modelo_rf_duzia or st.session_state.modelo_rf_coluna) and len(st.session_state.historico)>=tamanho_janela:
        try:
