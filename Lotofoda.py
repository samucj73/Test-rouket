...
# =====================================================
# ===== SISTEMA AUTÔNOMO COM ESTRATÉGIA HARD OTIMIZADA =====
# =====================================================
...
class SistemaAutonomo:
    ...
    AGORA COM ESTRATÉGIA HARD OTIMIZADA (Top 10 + 8 Atrasados + 5 Rotativos)
    """
    ...
    def __init__(self, concursos_historico):
        ...
        # Número de testes no backtest (padrão)
        self.num_testes = 50
    ...
    # ===== NOVA ESTRATÉGIA HARD OTIMIZADA =====
    def estrategia_hard_otimizado(self, qtd=18):
        """
        🔥 ESTRATÉGIA HARD OTIMIZADA
        Seleção inteligente baseada em:
        - Top 10 números mais frequentes (base fixa)
        - 8 números menos frequentes (atrasados)
        - Restante como rotativos
        Retorna os qtd números para a base
        """
        ...
        # Top 10 mais frequentes (base fixa)
        top_10 = [n for n, _ in freq.most_common(10)]
        
        # 8 menos frequentes (atrasados)
        menos_frequentes = sorted(freq.items(), key=lambda x: x[1])[:8]
        atrasados = [n for n, _ in menos_frequentes]
        
        # Rotativos = resto
        rotativos = list(set(range(1, 26)) - set(top_10) - set(atrasados))
        
        # Para formar a base de qtd números, combinar:
        # - Todos os top_10 (10 números)
        # - Complementar com atrasados e rotativos conforme necessário
        base = top_10.copy()
        
        # Se precisar de mais números, adicionar atrasados
        if len(base) < qtd:
            faltam = qtd - len(base)
            base.extend(atrasados[:faltam])
        
        # Se ainda faltar, adicionar rotativos
        if len(base) < qtd:
            faltam = qtd - len(base)
            base.extend(random.sample(rotativos, min(faltam, len(rotativos))))
        
        # Garantir que todos são únicos
        base = sorted(list(set(base)))
        
        # Se ainda não atingiu qtd, completar com números aleatórios
        while len(base) < qtd:
            novo = random.randint(1, 25)
            if novo not in base:
                base.append(novo)
        
        return sorted(base[:qtd])
    
    # ===== GERADOR DE JOGOS HARD =====
    def gerar_jogo_hard(self, base, rotativos, atrasados, max_tentativas=1000):
        """
        Gera um jogo usando a estrutura HARD:
        - Base fixa (10 números mais frequentes)
        - 5 números dos rotativos
        - Garantia de pelo menos 2 atrasados
        """
        def valido(jogo):
            pares = sum(1 for n in jogo if n % 2 == 0)
            
            # Distribuição por linhas do volante (5x5)
            linhas = [0] * 5
            for n in jogo:
                linhas[(n-1)//5] += 1
            ...
            return (
                ...
                1 <= linhas[3] <= 3 and
                3 <= linhas[4] <= 5 and
                len(set(jogo) & set(atrasados)) >= 2  # Pelo menos 2 atrasados
            )
        
        for _ in range(max_tentativas):
            # Selecionar 5 números dos rotativos (se disponível)
            if len(rotativos) >= 5:
                selecionados = random.sample(rotativos, 5)
            else:
                # Se não houver rotativos suficientes, completar com aleatórios
                selecionados = random.sample([n for n in range(1, 26) if n not in base], 5)
            
            jogo = sorted(base + selecionados)
        ...
        # Fallback: gerar jogo válido de forma mais simples
        for _ in range(max_tentativas):
            # Usar base completa + complemento aleatório
            complemento = random.sample([n for n in range(1, 26) if n not in base], 5)
            jogo = sorted(base + complemento)
            jogo = sorted(list(set(jogo)))
            
            if len(jogo) == 15 and valido(jogo):
                return jogo
    ...
    def avaliar_estrategia(self, estrategia_func, num_testes=None):
        """
        ...
        for i in range(50, min(50 + num_testes, self.total_concursos - 1)):
            ...
            # Se a estratégia espera concursos, passa o histórico
            if estrategia_func.__name__ in ['estrategia_hard_otimizado']:
    ...
    # ===== BACKTEST PARA ESTRATÉGIA HARD (usa gerador específico) =====
    def avaliar_estrategia_hard(self, num_testes=None):
        """
        Avaliação específica para a estratégia HARD que usa gerador especial
        """
        if self.total_concursos < 100:
            return 0
        ...
        for i in range(50, min(50 + num_testes, self.total_concursos - 1)):
            ...
            jogos = []
            for _ in range(10):
                jogo = self.gerar_jogo_hard(top_10, rotativos, atrasados)
                if jogo:
                    jogos.append(jogo)
    ...
    def escolher_melhor_estrategia(self, progress_callback=None):
        ...
        estrategias = {
            ...
            "🔥 HARD Otimizado": None  # Tratamento especial
        }
        ...
        # Testar estratégia HARD especial
        if progress_callback:
            progress_callback(idx / total_estrategias, "Testando 🔥 HARD Otimizado...")
        
        score_hard = self.avaliar_estrategia_hard()
        scores["🔥 HARD Otimizado"] = score_hard
        detalhes["🔥 HARD Otimizado"] = {
            "score": score_hard,
            "func": self.estrategia_hard_otimizado,
            "gerador": self.gerar_jogo_hard,
            "tipo": "hard"
        }
        ...
        melhor_tipo = detalhes[melhor_nome].get("tipo", "normal")
        
        return melhor_nome, melhor_func, melhor_score, scores, melhor_tipo
    ...
    def sistema_autonomo_completo(self, qtd_jogos=10, progress_callback=None):
        ...
        melhor_nome, melhor_func, melhor_score, todos_scores, melhor_tipo = self.escolher_melhor_estrategia(progress_callback)
        
        # Passo 2: Gerar base de acordo com o tipo
        base = None
        rotativos = None
        atrasados = None
        
        if melhor_tipo == "hard":
            # Para estratégia HARD, precisamos dos componentes
            freq = Counter()
            for c in self.concursos:
                freq.update(c)
            
            base = [n for n, _ in freq.most_common(10)]
            atrasados = [n for n, _ in sorted(freq.items(), key=lambda x: x[1])[:8]]
            rotativos = list(set(range(1, 26)) - set(base) - set(atrasados))
        else:
        ...
        if melhor_tipo == "hard":
            # Usar gerador HARD
            for _ in range(qtd_jogos * 2):  # Tentar mais vezes
                jogo = self.gerar_jogo_hard(base, rotativos, atrasados)
                if jogo and jogo not in jogos:
                    jogos.append(jogo)
                if len(jogos) >= qtd_jogos:
                    break
        else:
            # Usar gerador base normal
            jogos = self.gerar_jogos_base(base, qtd=qtd_jogos)
        
        return {
            "melhor_estrategia": melhor_nome,
            "melhor_score": melhor_score,
            "tipo_estrategia": melhor_tipo,
            "todos_scores": todos_scores,
            "base_utilizada": sorted(base) if base else [],
            "jogos": jogos,
...
def main():
        ...
        if st.button("📥 Carregar concursos", use_container_width=True):
            with st.spinner("Carregando dados da Caixa..."):
                try:
                    ...
                    # Inicializar sistema autônomo com a nova estratégia HARD
    ...
    if st.session_state.analise and st.session_state.dados_api and st.session_state.historico_df is not None:
        # AGORA SÃO 13 ABAS (adicionada a nova aba de IA 7.0)
        ...
        # =====================================================
        # ABA 12: SISTEMA AUTÔNOMO COM ESTRATÉGIA HARD
        # =====================================================
        with tab12:
            st.markdown("""
            <div style='background:#1e1e2e; padding:15px; border-radius:10px; margin-bottom:20px; border-left:5px solid #ff6600;'>
                <h4 style='margin:0; color:#ff6600;'>🤖 SISTEMA AUTÔNOMO HARD</h4>
                <p style='margin:5px 0 0 0; font-size:0.9em;'>Auto-estratégia com HARD Otimizado: Top 10 + 8 Atrasados + 5 Rotativos</p>
                <p style='margin:2px 0 0 0; font-size:0.85em; color:#ccc;'>Backtest automático • 5 estratégias concorrendo • A mais forte vence</p>
            </div>
            """, unsafe_allow_html=True)
            ...
            if st.session_state.dados_api and st.session_state.sistema_autonomo:
                ...
                with st.expander("📋 Entenda como funciona", expanded=False):
                    ...
                    1. **Testa 5 estratégias** em backtest:
                       ...
                       - 🔥 **HARD Otimizado:** Top 10 + 8 atrasados + 5 rotativos (NOVA!)
                ...
                # Botão principal
                if st.button("🚀 EXECUTAR SISTEMA AUTÔNOMO HARD", type="primary", use_container_width=True):
                ...
                if st.session_state.resultado_autonomo:
                    ...
                    st.markdown("---")
                    st.markdown("## 📊 RESULTADOS DO SISTEMA AUTÔNOMO HARD")
                    ...
                    cor_estrategia = "#ff6600" if "HARD" in resultado['melhor_estrategia'] else "#4ade80"
                    st.markdown(f"""
                    <div style='background:#1e1e2e; padding:20px; border-radius:15px; margin-bottom:20px; text-align:center; border:2px solid {cor_estrategia};'>
                        <h3 style='margin:0; color:{cor_estrategia};'>🏆 MELHOR ESTRATÉGIA</h3>
                        ...
                        <p style='color:#ff6600; font-size:0.9rem;'>🔥 Tipo: {resultado['tipo_estrategia']}</p>
                    </div>
                    """, unsafe_allow_html=True)
                    ...
                    if jogos_stats:  # Verificar se a lista não está vazia
                        ...
                        for i, jogo in enumerate(resultado['jogos']):
                            with st.container():
                                ...
                                # Cor especial para jogos HARD
                                cor_borda = "#ff6600" if "HARD" in resultado['melhor_estrategia'] else "#4ade80"
                        ...
                        with col3:
                            ...
                            st.download_button(
                                label="📥 Exportar CSV",
                                data=csv_autonomo,
                                file_name=f"jogos_autonomos_hard_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv",
                                mime="text/csv",
                                use_container_width=True
                            )
                    else:
                    ...
                    with st.expander("📘 Interpretando os Resultados"):
                        ...
                        **Sobre a Estratégia HARD:**
                        - **Top 10:** números mais frequentes (base fixa)
                        - **8 Atrasados:** números menos frequentes (frios)
                        - **5 Rotativos:** restante do volante
                        - **Garantia:** pelo menos 2 atrasados no jogo
        ...
        # =====================================================
        # ABA 13: IA 7.0 (Nível Profissional)
        # =====================================================
        with tab13:
            st.markdown("""
            <div style='background:#1e1e2e; padding:15px; border-radius:10px; margin-bottom:20px; border-left:5px solid #00ffaa;'>
                <h4 style='margin:0; color:#00ffaa;'>🤖 IA 7.0 (Nível Profissional)</h4>
                <p style='margin:5px 0 0 0; font-size:0.9em;'>Ranking + Diversificação + Atraso + Frequência</p>
                <p style='margin:2px 0 0 0; font-size:0.85em; color:#ccc;'>Geração em lote com pontuação inteligente • Eliminação de duplicatas</p>
            </div>
            """, unsafe_allow_html=True)
            ...
            with col2:
                st.markdown("<br>", unsafe_allow_html=True)
                if st.button("🚀 GERAR IA 7.0", use_container_width=True, type="primary"):
                    with st.spinner("Gerando jogos nível profissional..."):
                        ...
                        if jogos:
                            ...
                            if info:
                                st.info(f"📊 Base: Concurso {info[0]} - {info[1]}")
                        else:
                            st.error("❌ Erro ao gerar jogos. Verifique sua conexão com a internet.")
            ...
            if "jogos_ia" in st.session_state and st.session_state.jogos_ia:
                ...
                stats_df = pd.DataFrame({
                    ...
                    "Pares": [sum(1 for n in j if n % 2 == 0) for j in jogos],
                    "Soma": [sum(j) for j in jogos],
                    "Baixas": [sum(1 for n in j if n <= 8) for j in jogos],
                    "Médias": [sum(1 for n in j if 9 <= n <= 16) for j in jogos],
                    "Altas": [sum(1 for n in j if n >= 17) for j in jogos]
                })
                ...
                for i, jogo in enumerate(jogos):
                    ...
                    <div style='border-left: 5px solid #00ffaa; background:#0e1117; border-radius:10px; padding:15px; margin-bottom:10px;'>
                        <div style='display:flex; justify-content:space-between;'>
                            <strong>🏆 Jogo IA 7.0 #{i+1}</strong>
                            <small>⚖️ {pares} pares | ➕ {soma}</small>
                        </div>
                        <div>{nums_html}</div>
                    </div>
                    """, unsafe_allow_html=True)
                ...
                df_export = pd.DataFrame({
                    ...
                    "Pares": stats_df["Pares"],
                    "Soma": stats_df["Soma"],
                    "Baixas(1-8)": stats_df["Baixas"],
                    "Médias(9-16)": stats_df["Médias"],
                    "Altas(17-25)": stats_df["Altas"]
                })
                ...
                st.download_button(
                    "📥 Exportar CSV",
                    data=csv,
                    file_name=f"jogos_ia70_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv",
                    mime="text/csv",
                    use_container_width=True
                )
