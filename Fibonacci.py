...
def enviar_resultado_super_simplificado(numero_real, acerto, nome_estrategia, detalhes_acerto=None):
        ...
        if acerto:
            if 'Fibonacci' in nome_estrategia:
                ...
                if '+' in detalhes_acerto:
                    ...
                    # Apenas uma zona
                    if detalhes_acerto == 'Vermelha':
                        nucleo = "7"
                    elif detalhes_acerto == 'Azul':
                        nucleo = "10"
                    elif detalhes_acerto == 'Amarela':
                        nucleo = "2"
                    else:
                        nucleo = detalhes_acerto
                    mensagem = f"✅ Acerto Núcleo {nucleo}\n🎲 Número: {numero_real}"
            elif 'ML' in nome_estrategia and detalhes_acerto:
                # CORREÇÃO: Mostrar número do núcleo em vez do nome da zona
                if '+' in detalhes_acerto:
                    # Múltiplas zonas acertadas
                    zonas = detalhes_acerto.split('+')
...
class EstrategiaFibonacci:
    ...
    def gerar_numeros_aposta(self):
        ...
        if len(numeros_filtrados) > self.config['max_numeros_apostar']:
            numeros_filtrados = self.sistema_selecao.selecionar_melhores_15_numeros(
                numeros_filtrados, self.historico, "Fibonacci"
            )
        
        return sorted(numeros_filtrados)

    def analisar_fibonacci(self):
        """Análise principal da estratégia Fibonacci"""
        if len(self.historico) < self.config['min_historico']:
            return None
    ...
    def get_analise_detalhada(self):
        """Retorna análise detalhada da estratégia Fibonacci"""
        if len(self.historico) == 0:
    ...
    def zerar_estatisticas(self):
        """Zera todas as estatísticas Fibonacci"""
        self.stats = {
            'acertos': 0,
            'tentativas': 0,
            'sequencia_atual': 0,
...
# =============================
# SISTEMA DE GESTÃO ATUALIZADO COM ROTAÇÃO AUTOMÁTICA (com Fibonacci)
# =============================
class SistemaRoletaCompleto:
    def __init__(self):
        ...
        self.estrategia_fibonacci = EstrategiaFibonacci()  # NOVA ESTRATÉGIA
    ...
    def rotacionar_estrategia_automaticamente(self, acerto, nome_estrategia):
        ...
        if acerto:
            ...
            if self.sequencia_erros >= 2:
                ...
                # Ciclo de rotação incluindo Fibonacci
                if estrategia_atual == "Zonas":
                    nova_estrategia = "ML"
                elif estrategia_atual == "ML":
                    nova_estrategia = "Fibonacci"  # Adicionado Fibonacci
                elif estrategia_atual == "Fibonacci":
    ...
    def processar_novo_numero(self, numero):
        ...
        if self.previsao_ativa:
            ...
            enviar_resultado_super_simplificado(numero_real, acerto, nome_estrategia, detalhes_acerto)
            ...
            self.historico_desempenho.append({
                ...
                'detalhes': detalhes_acerto,
                'tipo_aposta': self.previsao_ativa.get('tipo', 'unica')
            })
            
            self.previsao_ativa = None
        
        # Processar número em todas as estratégias
        ...
        self.estrategia_fibonacci.adicionar_numero(numero_real)  # Nova estratégia
        ...
        elif self.estrategia_selecionada == "Fibonacci":  # Nova estratégia
            nova_estrategia = self.estrategia_fibonacci.analisar_fibonacci()
    ...
    def zerar_estatisticas_desempenho(self):
        ...
        self.estrategia_fibonacci.zerar_estatisticas()
...
estrategia = st.sidebar.selectbox(
    ...
    ["Zonas", "Midas", "ML", "Fibonacci"],  # Adicionado Fibonacci
...
with st.sidebar.expander("🔄 Rotação Automática", expanded=True):
    ...
    st.write("---")
    st.write("**Novo Ciclo de Rotação:**")
    ...
    st.write("• 🔄 **Zonas → ML → Fibonacci → Zonas** (ciclo completo)")
    ...
    if st.button("🔄 Forçar Rotação", use_container_width=True):
        ...
        # Novo ciclo de rotação
        if estrategia_atual == "Zonas":
            nova_estrategia = "ML"
        elif estrategia_atual == "ML":
            nova_estrategia = "Fibonacci"
        elif estrategia_atual == "Fibonacci":
            nova_estrategia = "Zonas"
        else:
            nova_estrategia = "Zonas"  # Fallback para Midas
...
with st.sidebar.expander(f"🔍 Análise - {estrategia}", expanded=False):
    ...
    elif estrategia == "Fibonacci":  # NOVA ANÁLISE
        analise = st.session_state.sistema.estrategia_fibonacci.get_analise_detalhada()
    else:
