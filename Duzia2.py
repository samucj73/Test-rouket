...
if len(historico) == 0 or numero_atual != historico[-1]:
    ...
    if modelo_duzia and modelo_coluna:
        ...
        prob_min_duzia = max(0.55, 0.60 if taxa_duzia > 0.5 else 0.55)
        prob_min_coluna = max(0.55, 0.60 if taxa_coluna > 0.5 else 0.55)
        ...
        # Evita repetir previsão igual ao número atual
        ...
        if (duzia, coluna) != st.session_state.ultimo_alerta and (duzia or coluna):
            ...
            if duzia: mensagem += f" | D: <b>{duzia}</b>"
            if coluna: mensagem += f" | C: <b>{coluna}</b>"
...
st.write("Acertos Dúzia:", st.session_state.acertos_duzia, "/", st.session_state.total_duzia)
st.write("Acertos Coluna:", st.session_state.acertos_coluna, "/", st.session_state.total_coluna)
